...
def UCT_search(game_state, num_reads, net, gpu_id):
...
def MCTS_self_play(chessnet,num_games,cpu,gpu_id):
    for idxx in range(0,num_games):
        ...
        while checkmate == False and current_board.move_count <= 100:
            ...
            best_move, root = UCT_search(current_board,777,chessnet,gpu_id)
...
if __name__=="__main__":
    ...
    for i in range(12):
        gpu_id = i % 2
        p = mp.Process(target=MCTS_self_play,args=(net,50,i,gpu_id))
        p.start()
        processes.append(p)
    for p in processes:
        p.join()
