...
def get_model_move(board, model, move_history):
    ...
    model_input = np.stack(list(move_history))
    model_input = torch.tensor(model_input, dtype=torch.float32)

    # model_input = prepare_model_input(board, move_history)
    move_probs = model.predict(model_input) # Modify according to your model's prediction method

    # Generate mask for legal moves
    legal_moves = board.legal_moves
    legal_move_mask = np.zeros_like(move_probs, dtype=bool)
    ...
    # Apply mask
    masked_probs = np.logical_and(move_probs, legal_move_mask)

    # Normalize probabilities
    normalized_probs = softmax(masked_probs)

    # Select move
    selected_move_index = np.argmax(normalized_probs)
